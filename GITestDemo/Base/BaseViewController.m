...
-(void)downloadAndTransferParams{
...
- (void)initBLESDK{
    ...
    if (!shangHuName) {
        [[NSUserDefaults standardUserDefaults] setObject:@"立得支付" forKey:kShangHuName];
    }
    [[NSUserDefaults standardUserDefaults]synchronize];
...
//    }
    ...
    if ([self.statusStr isEqualToString:[NSString stringWithFormat:@"签到失败"]]) {
        ...
        [self downloadAndTransferParams];
    ...
    if ([self.statusStr isEqualToString:@"获取设备信息成功"] ) {
        ...
        if (_isFirstGetVersionInfo){
            ...
            [KUserDefault setObject:pos_task forKey:kMposG1task];
            [KUserDefault setObject:pos_kernel forKey:kMposG1kernel];
